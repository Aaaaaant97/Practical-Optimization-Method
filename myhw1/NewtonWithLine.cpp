...
double LineSearch(Vector4d x,Vector4d p){
        double alpha=0.8,c=1e-4;
        Vector4d y=x+alpha*p;
        while(f(y)>f(x)+c*alpha*(grad(x)(0)*p(0)+grad(x)(1)*p(1)+grad(x)(2)*p(2)+grad(x)(3)*p(3))){
                alpha*=0.618;
                y=x+alpha*p;
        }
        return alpha;
